...
struct EditGradeView: View {
    ...
    @State private var gradeValue: Double
    ...
    init(grade: Grade) {
        self.gradeToEdit = grade
        self._selectedSubject = State(initialValue: grade.subject)
        self._gradeValue = State(initialValue: grade.value)
        self._description = State(initialValue: grade.description)
        self._selectedDate = State(initialValue: grade.date)
    }
        ...
        ZStack {
            ...
            HStack {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    Text("Aggiorna i dettagli del voto esistente")
                        .font(.caption)
    ...
    private func saveGrade() {
        ...
        let updatedGrade = Grade(
            ...
            teacher: dataManager.getTeacherFor(subject: selectedSubject),
            color: dataManager.getColorFor(subject: selectedSubject)
        )
        
        // Rimuovo il vecchio voto e aggiungo quello aggiornato
        dataManager.deleteGrade(gradeToEdit)
        dataManager.addGrade(updatedGrade)
        dismiss()
    }
...
#Preview {
    // Preview con dati mock per il test
    let mockGrade = Grade(
        subject: "Matematica",
        value: 7.5,
        date: Date(),
